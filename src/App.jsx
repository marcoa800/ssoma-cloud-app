...
import {
  ...
  FileSpreadsheet, File as FileIcon, Filter, LogOut, RefreshCw,
  Loader2
} from 'lucide-react';
import { CLIENT_ID } from './config.js';
import {
  initSheets, loadAllData,
...
const LoginScreen = ({ onLogin, isLoading }) => (
  <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-blue-50">
    <div className="text-center max-w-sm w-full px-6">
      <div className="bg-blue-600 p-5 rounded-3xl inline-flex mb-6 shadow-2xl shadow-blue-300">
        <Shield size={44} className="text-white" />
      </div>
      <h1 className="text-3xl font-black text-slate-800 mb-1">SSOMA Cloud</h1>
      ...
      <button
        onClick={onLogin}
        disabled={isLoading}
        className="w-full bg-white border border-slate-200 px-6 py-4 rounded-2xl font-bold text-slate-700 shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-3 disabled:opacity-60"
      >
        {isLoading ? (
      ...
      <p className="text-[11px] text-slate-400 mt-4">
        Tus datos se guardan en tu Google Sheet personal
      </p>
    </div>
  </div>
);
...
export default function App() {
  ...
  const [data, setData] = useState({ workers: [], kpis: [], documents: [] });
  ...
  const fetchData = useCallback(async (accessToken) => {
    ...
    setError(null);
    try {
      const result = await loadAllData(accessToken);
      setData(result);
    } catch (e) {
    ...
    const setup = () => {
      ...
      const client = window.google.accounts.oauth2.initTokenClient({
        ...
        callback: async (response) => {
          ...
          setDataLoading(true);
          try {
            await initSheets(accessToken);
            await fetchData(accessToken);
          } catch (e) {
            setError('Error iniciando hojas: ' + e.message);
            setDataLoading(false);
          }
        },
      });
  ...
  const handleLogout = () => {
    ...
    setToken(null);
    setData({ workers: [], kpis: [], documents: [] });
  };

  // ── Handler genérico para errores de API ────────────────────
  ...
  const handleAddWorker = async (workerData) => {
    ...
    await withApi(async () => {
      await addWorker(token, newWorker);
  ...
  const handleAddKPI = async (kpiData) => {
    const newKpi = { ...kpiData, id: Date.now(), value: Number(kpiData.value), target: Number(kpiData.target) };
    await withApi(async () => {
      await addKPI(token, newKpi);
  ...
  const handleAddDoc = async (docData) => {
    const newDoc = { ...docData, id: Date.now() };
    await withApi(async () => {
      await addDocument(token, newDoc);
      setData((prev) => ({ ...prev, documents: [newDoc, ...prev.documents] }));
      setModalOpen(null);
    });
  };

  // ── Pantallas de auth / carga ────────────────────────────────

  if (!token) {
    return <LoginScreen onLogin={handleLogin} isLoading={authLoading} />;
  }

  if (dataLoading) {
    return <LoadingScreen message="Sincronizando con Google Sheets..." />;
  }
      ...
      <div className="space-y-6 animate-in fade-in">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <Card className="border-l-4 border-l-purple-600">
            ...
            <div className="flex items-end justify-between mt-2">
              <span className="text-4xl font-black text-slate-800">
                {activeWorkers.filter((w) => w.aptitude === 'Apto con Restricción').length}
              </span>
          ...
          <Card className="border-l-4 border-l-amber-500">
            ...
            <div className="flex items-end justify-between mt-2">
              <span className="text-4xl font-black text-slate-800">
                {activeWorkers.filter((w) => new Date(w.emoExp) < new Date(new Date().setDate(new Date().getDate() + 30))).length}
              </span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card>
            <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2">
              <TrendingUp size={18} /> Resumen de KPIs Recientes
            </h3>
            <div className="space-y-3">
              {data.kpis.slice(0, 4).map((k) => (
                <div key={k.id} className="flex justify-between items-center p-3 bg-slate-50 rounded-xl">
                  ...
                  <Badge type={k.value <= k.target ? 'success' : 'danger'}>
                    {k.value} (Meta: {k.target})
                  </Badge>
                </div>
              ))}
              {data.kpis.length === 0 && (
                <p className="text-sm text-slate-400 text-center py-4">Sin KPIs registrados</p>
              )}
            </div>
          </Card>
          <Card>
            <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2">
              <Shield size={18} /> Cumplimiento SST (Activos)
            </h3>
  ...
  const MasterWorkerDirectory = () => {
    ...
    const [filterAptitude, setFilterAptitude] = useState('Todos');
    ...
    return (
      <div className="space-y-6 animate-in slide-in-from-bottom-4">
        ...
        <div className="grid grid-cols-1 gap-4">
          ...
          {filtered.length === 0 && (
            <div className="text-center py-10 text-slate-400 text-sm">No se encontraron trabajadores</div>
          )}
        </div>
      </div>
    );
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredDocs.map((doc) => (
                <tr key={doc.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className="text-[10px] text-slate-400">Registro en Sheets</span>
                  </td>
                </tr>
              ))}
              {filteredDocs.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-6 py-8 text-center text-slate-400 text-sm">Sin documentos en esta área</td>
                </tr>
              )}
            </tbody>
          </table>
  ...
  const WorkerModal = () => {
    ...
    const onSave = async () => {
      if (!form.name || !form.dni || !form.emoDate) {
        alert('Completa los campos obligatorios: Nombre, DNI y Fecha EMO');
        return;
      }
      setSaving(true);
      await handleAddWorker(form);
      setSaving(false);
    };
      ...
      <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl w-full max-w-lg p-6 shadow-2xl overflow-y-auto max-h-[90vh]">
          ...
          <div className="flex gap-2 mt-6">
            ...
            <button onClick={onSave} disabled={saving} className="flex-1 py-3 bg-blue-600 text-white rounded-lg font-bold text-xs uppercase shadow-lg shadow-blue-200 disabled:opacity-60 flex items-center justify-center gap-2">
  ...
  const DocModal = () => {
    ...
    const handleFileChange = (e) => {
      ...
      if (file) {
        ...
        setForm({ ...form, name: file.name, type, file });
      }
    };

    const onSave = async () => {
      if (!form.name) { alert('Selecciona un archivo o ingresa un nombre'); return; }
      setSaving(true);
      await handleAddDoc(form);
      setSaving(false);
    };
    ...
    return (
      <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-2xl">
          <h3 className="font-bold text-lg mb-4">Registrar Documento</h3>
          <div className="space-y-4">
            <div className="border-2 border-dashed border-slate-300 rounded-xl p-6 text-center hover:bg-slate-50 cursor-pointer relative">
              ...
              <p className="text-xs font-bold text-slate-600">{form.file ? form.name : 'Click para seleccionar archivo'}</p>
              <p className="text-[10px] text-slate-400 mt-1">El nombre se registrará en Sheets</p>
            </div>
            <div className="grid grid-cols-2 gap-3">
          ...
          <div className="flex gap-2 mt-6">
            ...
            <button onClick={onSave} disabled={saving} className="flex-1 py-2 bg-emerald-600 text-white rounded-lg font-bold text-xs uppercase disabled:opacity-60 flex items-center justify-center gap-2">
              {saving && <Loader2 size={14} className="animate-spin" />}
  ...
  const KPIModal = () => {
    ...
    const onSave = async () => {
      if (!form.metric || form.value === '' || form.target === '') {
        alert('Completa todos los campos del KPI');
        return;
      }
      setSaving(true);
      await handleAddKPI(form);
      setSaving(false);
    };
    ...
    return (
      <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-2xl">
          ...
          <div className="flex gap-2 mt-6">
            <button onClick={() => setModalOpen(null)} className="flex-1 py-2 text-slate-500 font-bold text-xs uppercase">Cancelar</button>
            <button onClick={onSave} disabled={saving} className="flex-1 py-2 bg-slate-900 text-white rounded-lg font-bold text-xs uppercase disabled:opacity-60 flex items-center justify-center gap-2">
              {saving && <Loader2 size={14} className="animate-spin" />}
              {saving ? 'Guardando...' : 'Guardar en Sheets'}
            </button>
          </div>
  ...
  return (
    <div className="flex h-screen bg-[#f8fafc] text-slate-900 font-sans overflow-hidden">
      ...
      {!isSidebarOpen && (
        <div className="fixed inset-0 bg-black/20 z-10 md:hidden" onClick={() => setSidebarOpen(true)} />
      )}
      ...
      <aside className={`w-64 bg-white border-r border-slate-200 flex flex-col fixed md:relative z-20 h-full transition-all duration-300 ${!isSidebarOpen ? '-translate-x-full md:translate-x-0 md:w-20' : ''}`}>
        <div className="p-6 h-20 flex items-center gap-3 border-b border-slate-50">
          ...
          {isSidebarOpen && <span className="font-bold text-xs uppercase tracking-widest text-slate-800">SSOMA Cloud</span>}
        </div>
        ...
        {/* Sheets indicator + logout */}
        {isSidebarOpen && (
          <div className="p-4 border-t border-slate-100">
            <div className="bg-emerald-50 rounded-xl p-3 mb-3 flex items-center gap-2">
              <div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse" />
              <span className="text-[10px] font-bold text-emerald-700 uppercase">Google Sheets activo</span>
            </div>
            <button onClick={handleLogout} className="w-full flex items-center gap-2 p-2 rounded-xl text-xs text-slate-400 hover:text-red-500 hover:bg-red-50 transition-all">
          ...
          <div className="flex items-center gap-4">
            ...
            <h1 className="font-bold text-slate-800 uppercase tracking-widest text-xs hidden md:block">
              {activeTab === 'dashboard' ? 'Panel Principal' :
               ...
               activeTab === 'docs' ? 'Archivo Digital' : ''}
            </h1>
          </div>
          <div className="flex items-center gap-3">
            <button
              onClick={() => fetchData(token)}
              title="Recargar desde Sheets"
              className="text-slate-400 hover:text-blue-500 transition-colors p-2 rounded-lg hover:bg-blue-50"
            >
              <RefreshCw size={16} />
            </button>
          ...
          {activeTab === 'kpis' && (
            <div className="space-y-6 animate-in slide-in-from-right-4">
              <div className="flex justify-between items-center">
                ...
                <button onClick={() => setModalOpen('kpi')} className="bg-slate-900 text-white px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2 hover:bg-slate-800">
                  <Plus size={16} /> Métrica
                </button>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {data.kpis.map((k) => (
                ...
                {data.kpis.length === 0 && (
                  <div className="col-span-3 text-center py-10 text-slate-400 text-sm">
                    Sin KPIs registrados. Haz clic en "Métrica" para agregar.
                  </div>
                )}
              </div>
            </div>
